...
给定一个子单纯形 $f\in \Delta_{\ell}(T)$，下面介绍~\cite{ChenChenHuangWei2024} 中构造的两组 $\mathscr N^f$ 的基. 
...
记边向量 $\bs t_{i,j}:=\textnormal{\texttt{v}}_j-\textnormal{\texttt{v}}_i$， 其中 $\textnormal{\texttt{v}}_0, \textnormal{\texttt{v}}_1, \cdots, \textnormal{\texttt{v}}_d$ 是单纯形 $T$ 的顶点. 成立
\begin{equation}\label{eq:tangradlambdadual}
\bs t_{i,j}\cdot\nabla\lambda_{\ell}=\delta_{j,\ell}-\delta_{i,\ell}, \quad \forall\,i,j,\ell=0,1,\ldots,d.
\end{equation}
