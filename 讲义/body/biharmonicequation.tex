...
\[
H(\div{\div },\Omega;
\mathbb{S}):=\{\boldsymbol{\tau}\in L^{2}(\Omega; \mathbb{S}): \div \div\boldsymbol{\tau}\in L^2(\Omega)\},
\]
其范数平方定义为 
$
...
&- \sum_{F\in\partial K}\left[( \tr_1^{\div\div}(\bs \sigma), \tr_1^{\nabla^2}(v))_{F} -  ( \tr_2^{\div\div}(\bs \sigma), \tr_2^{\nabla^2}(v))_F\right] -\sum_{e\in\Delta_{d-2}(K)}(\tr_e^{\div\div}(\bs \sigma), \tr_e^{\nabla^2}(v))_e, \label{eq:greenidentitydivdiv} 
\end{align}
其中
\begin{align*}
&\tr_1^{\div\div}(\bs \sigma) = \boldsymbol  n_{\partial K}^{\intercal}\boldsymbol \sigma\boldsymbol  n_{\partial K}, &  & \tr_1^{\nabla^2}( v) = \partial_n v\mid_{\partial K},\\
&\tr_2^{\div\div}(\bs \sigma) =  \boldsymbol n_{\partial K}^{\intercal}\div \boldsymbol \sigma +  \div_F(\boldsymbol\sigma \boldsymbol n_{\partial K}),&  & \tr_2^{\nabla^2}(v) = v\mid_{\partial K},\\
&\tr_e^{\div\div}(\bs \sigma) = \sum_{F\in\partial K,e\in \partial F}\boldsymbol n_{F,e}^{\intercal}\boldsymbol \sigma \boldsymbol n_{\partial K}, &  & \tr_e^{\nabla^2}(v) = v \mid_{\Delta_{d-2}(K)}.
\end{align*}
\end{lemma}
\begin{proof}
应用分部积分公式可得：
...
设 $\mathcal{T}_h$ 是 $\Omega$ 的一个多面体网格剖分. 记 $\Delta_{\ell}(\mathcal{T}_h)$ 和 $\Delta_{\ell}(\mathring{\mathcal{T}}_h)$ 分别是 $\mathcal{T}_h$ 中所有 $\ell$ 维面和内部 $\ell$ 维面的集合，并令 
$$
\mathcal{F}_h:=\Delta_{d-1}(\mathcal{T}_h),\quad \mathcal{E}_h:=\Delta_{d-2}(\mathcal{T}_h),\quad \mathring{\mathcal F}_h:=\Delta_{d-1}(\mathring{\mathcal{T}}_h),\quad \mathring{\mathcal E}_h:=\Delta_{d-2}(\mathring{\mathcal{T}}_h).
$$
% 记 $\mathcal{F}_h$ 和 $\mathcal{E}_h$ 分别是 $\mathcal{T}_h$ 中所有 $(d-1)$ 维面和 $(d-2)$ 维面的集合，并分别记 $\mathring{\mathcal F}_h$ 和 $\mathring{\mathcal E}_h$ 为其内部$(d-1)$ 维面和内部$(d-2)$ 维面的集合. 
当在所有单元上求和时，假设测试函数 $v$ 足够光滑（例如 $v\in C^2(\Omega)$），我们可以合并内部面和棱上的项. 对于内部面 $F\in \mathring{\mathcal F}_h$，记包含 $F$ 的两个单元为 $K_1, K_2$. 
引入如下跳量定义：
\begin{align*}
&[\tr_1^{\div\div}(\bs \sigma)]_F : = \bs n_{\partial K_1}^{\intercal}\bs \sigma \bs n_{\partial K_1} \mid _{F} - \bs n_{\partial K_2}^{\intercal}\bs \sigma \bs n_{\partial K_2} \mid _{F},  \\
&[\tr_2^{\div\div}(\bs \sigma)]_F := (\boldsymbol n_{\partial K_1}^{\intercal}\div \boldsymbol \sigma +  \div_F(\boldsymbol\sigma \boldsymbol n_{\partial K_1}))\mid _{F} + (\boldsymbol n_{\partial K_2}^{\intercal}\div \boldsymbol \sigma +  \div_F(\boldsymbol\sigma \boldsymbol n_{\partial K_2}))\mid _{F},  &  &\\
& [ \tr_e^{\div\div}(\bs \sigma)]|_e := \sum_{K\in \omega_e} \sum_{F\in\partial K, e\in\partial F} (\boldsymbol n_{F,e}^{\intercal}\boldsymbol \sigma \boldsymbol n_{\partial K})|_e.  &  &
\end{align*}
...
&\;=\{g\in L^2(\partial K): g|_F\in H_{00}^{1/2}(F)\;\;\forall~F\in\Delta_{d-1}(K)\},
\end{align*}
其范数为
\[
\|g\|_{H_{n,0}^{1/2}(\partial K)}:=\inf_{v\in H^2(K)\cap H_0^1(K), \; (\partial_n v)|_{\partial K}=g}\|v\|_{2,K},
\]
以及
\begin{align*}
H_{t,0}^{3/2}(\partial K)&:=\{v|_{\partial K}: v\in H^2(K), (\partial_n v)|_{\partial K}=0, v|_{e}=0\;\;\forall~e\in\Delta_{d-2}(K) \},
\end{align*}
其范数为
\[
\|g\|_{H_{t,0}^{3/2}(\partial K)}:=\inf_{v\in H^2(K)\atop (\partial_n v)|_{\partial K}=0, v|_{\partial K}=g}\|v\|_{2,K}.
\]
对于 $\tr_1^{\div\div}$，令 $H_n^{-1/2}(\partial K):=(H_{n,0}^{1/2}(\partial K))'$；对于 $\tr_2^{\div\div}$，令 $H_t^{-3/2}(\partial K):=(H_{t,0}^{3/2}(\partial K))'$.


%\begin{lemma}
%For any $\boldsymbol \tau\in H(\div \boldsymbol{\div },K; \mathbb{S})$,  it holds
%\[
%\|\boldsymbol  n^{\intercal}\boldsymbol \tau\boldsymbol  n\|_{H_n^{-1/2}(\partial K)} \lesssim \|\boldsymbol{\tau}\|_{ H(\div \boldsymbol{\div })}.
%\]
%Conversely, for any $g_n\in H_n^{-1/2}(\partial K)$, there exists some $\boldsymbol \tau\in H(\div \boldsymbol{\div },K; \mathbb{S})$ such that
%\[
%\boldsymbol  n^{\intercal}\boldsymbol \tau\boldsymbol  n|_{\partial K}=g_n, \quad 
%\|\boldsymbol{\tau}\|_{ H(\div \boldsymbol{\div })} \lesssim \|g_n\|_{H_n^{-1/2}(\partial K)}.
%\]
...
对于任意 $\boldsymbol \tau\in H(\div{\div },K; \mathbb{S})$，成立：
\[
\|\boldsymbol n^{\intercal}\boldsymbol \tau\boldsymbol n\|_{H_n^{-1/2}(\partial K)} + \|2\div_F(\boldsymbol\tau \boldsymbol n)+ \partial_n(\boldsymbol n^{\intercal} \boldsymbol \tau\boldsymbol n)\|_{H_t^{-3/2}(\partial K)}\lesssim \|\boldsymbol{\tau}\|_{ H(\div{\div },K)}.
\]
反之，对于任意 $g_n\in H_n^{-1/2}(\partial K)$ 和 $g_t\in H_t^{-3/2}(\partial K)$，存在 $\boldsymbol \tau\in H(\div{\div },K; \mathbb{S})$ 使得
\[
...
且满足
\[
\|\boldsymbol{\tau}\|_{ H(\div{\div },K)} \lesssim \|g_n\|_{H_n^{-1/2}(\partial K)}+\|g_t\|_{H_t^{-3/2}(\partial K)},
\]
其中的隐含常数仅依赖于区域 $K$ 的形状.
\end{lemma}
...
注意，格林公式~\eqref{eq:greenidentitydivdiv} 中的项 $(\boldsymbol n_{F,e}^{\intercal}\boldsymbol \tau \boldsymbol n, v)_e$ 并未被引理~\ref{lem:Hdivdivtrace} 所覆盖. 事实上，$ H(\div{\div },K; \mathbb{S})$ 的迹的完整刻画由 $(\div{\div }\bs\tau, v)_K-\left(\boldsymbol \tau, \nabla^{2} v\right)_{K}$ 给出，而该表达式无法被等价地解耦（参见文献~\cite[引理~3.2]{FuehrerHeuerNiemi2019}）.
然而，在施加额外光滑性假设的情况下，可以将此迹在各面上局部化.


下面给出分段光滑函数属于 $ H(\div{\div },\Omega; \mathbb{S})$ 的一个充分条件. 
...
\begin{enumerate}
\item $[\tr_1^{\div\div}(\bs \sigma)]_F = 0$ 对于所有 $F\in \mathring{\mathcal F}_h$ 成立；
\smallskip
\item $[\tr_2^{\div\div}(\bs \sigma)]_F = 0$ 对于所有 $F\in \mathring{\mathcal F}_h$ 成立；
\smallskip
\item $[ \tr_e^{\div\div}(\bs \sigma)]|_e = 0$ 对于所有 $e\in \mathring{\mathcal E}_h$ 成立. 
\end{enumerate}
\end{lemma}


在构造 $H(\operatorname{div}\operatorname{div})$-协调有限元时，强制跳跃条件
$[ \operatorname{tr}_e^{\div\div}(\boldsymbol{\sigma}) ]|_e = 0$
...
H^{-1}(\div{\div },\Omega;\mathbb{S}) = \iota(H_0^1(\Omega)) \oplus \big(L^{2}(\Omega; \mathbb{S})\cap \ker(\div\div)\big),
\end{equation}
其中 $\iota (v) = v \boldsymbol I_{d\times d}$，以及 
$$
L^{2}(\Omega; \mathbb{S})\cap \ker(\div\div)=\{\boldsymbol{\tau}\in L^{2}(\Omega; \mathbb{S}): \div\div\boldsymbol{\tau}=0\}
$$ 
\end{lemma}
\begin{proof}
...
若 $\Omega$ 为可缩区域，则核空间 $L^{2}(\Omega; \mathbb{S})\cap \ker(\operatorname{div}\operatorname{div})$ 可被显式刻画.
...
% 记 $\Gamma:=\partial\Omega$.
% 令
% \[
% H_{n,0}^{1/2}(\Gamma):=\{(\partial_{n}\varphi)|_{\Gamma}: \varphi\in H^2(\Omega)\cap H_0^1(\Omega)\},
% \]
% 并赋予范数
% \[
% \|g\|_{H_{n,0}^{1/2}(\Gamma)}:=\inf_{\varphi\in H^2(\Omega)\cap H_0^1(\Omega), (\partial_{n}\varphi)|_{\Gamma}=g}\|\varphi\|_{H^2(\Omega)}.
% \]
...
\begin{align*}
\|\boldsymbol{n}^{\intercal}\bs\tau\boldsymbol{n}\|_{H_{n}^{-1/2}(\Gamma)}&=\sup_{g\in H_{n,0}^{1/2}(\Gamma)}\frac{\langle \boldsymbol{n}^{\intercal}\bs\tau\boldsymbol{n}, g\rangle}{\|g\|_{H_{n,0}^{1/2}(\Gamma)}}=\sup_{\varphi\in H^2(\Omega)\cap H_0^1(\Omega)}\frac{\langle \boldsymbol{n}^{\intercal}\bs\tau\boldsymbol{n}, \partial_{n}\varphi\rangle}{\|\varphi\|_{H^2(\Omega)}}.
\end{align*}
...
\begin{lemma}
如下等式成立：
\begin{equation}\label{divdivHn1Sonto}
    \operatorname{div}\operatorname{div} H^{-1}(\operatorname{div}\operatorname{div}, \Omega; \mathbb{S}) = H^{-1}(\Omega),
\end{equation}
此外，如下 inf-sup 条件成立：
\begin{equation}\label{eq:divdivn1infsup}
    \|v\|_1 \lesssim \sup_{\boldsymbol{\tau} \in H^{-1}(\operatorname{div}\operatorname{div}, \Omega; \mathbb{S})} \frac{\langle \operatorname{div}\operatorname{div} \boldsymbol{\tau}, v\rangle}{\|\boldsymbol{\tau}\|_{H^{-1}(\operatorname{div}\operatorname{div})}}, \quad \forall~v \in H_0^1(\Omega).
\end{equation}
\end{lemma}
\begin{proof}
由直和分解 \eqref{Hn1divdivSdecomp} 可知，
\[
    \operatorname{div}\operatorname{div} H^{-1}(\operatorname{div}\operatorname{div}, \Omega; \mathbb{S}) = \operatorname{div}\operatorname{div}(\iota(H_0^1(\Omega))) = \Delta H_0^1(\Omega).
\]
结合 $\Delta H_0^1(\Omega) = H^{-1}(\Omega)$，可知 \eqref{divdivHn1Sonto} 成立. 

接下来证明 inf-sup 条件 \eqref{eq:divdivn1infsup}. 
对于任意 $v \in H_0^1(\Omega)$，通过计算可得：
\[
    \langle \operatorname{div}\operatorname{div}(-\iota(v)), v\rangle = -\langle \operatorname{div}\operatorname{div}(\iota(v)), v\rangle = -\langle \Delta v, v\rangle = |v|_1^2,
\]
且
\[
    \|-\iota(v)\|_{H^{-1}(\operatorname{div}\operatorname{div})}^2 = \|\iota(v)\|^2 + |v|_1^2 \lesssim |v|_1^2.
\]
由此可得：
\[
    \sup_{\boldsymbol{\tau} \in H^{-1}(\operatorname{div}\operatorname{div}, \Omega; \mathbb{S})} \frac{\langle \operatorname{div}\operatorname{div} \boldsymbol{\tau}, v\rangle}{\|\boldsymbol{\tau}\|_{H^{-1}(\operatorname{div}\operatorname{div})}} 
    \geq \frac{\langle \operatorname{div}\operatorname{div}(-\iota(v)), v\rangle}{\|-\iota(v)\|_{H^{-1}(\operatorname{div}\operatorname{div})}} 
    \gtrsim |v|_1.
\]
故 inf-sup 条件 \eqref{eq:divdivn1infsup} 成立. 
\end{proof}


\begin{theorem}[推论 2.3，文献 \cite{KrendlRafetsederZulehner2016}]
分布混合变分形式 \eqref{eq:hhjmixedformulation} 是适定的，且与问题 \eqref{eq:biharmonicvariatonalform} 等价. 即，若 $u\in H_0^2(\Omega)$ 是问题 \eqref{eq:biharmonicvariatonalform} 的解，则 $\bs\sigma=-\nabla^2 u\in H^{-1}(\div\div,\Omega; \mathbb{S})$ 且 $(\bs\sigma, u)$ 是分布混合变分形式 \eqref{eq:hhjmixedformulation} 的解. 反之，若 $(\bs\sigma, u)\in H^{-1}(\div\div,\Omega; \mathbb{S})\times H_0^1(\Omega)$ 是分布混合变分形式 \eqref{eq:hhjmixedformulation} 的解，则 $u\in H_0^2(\Omega)$ 且 $u$ 是问题 \eqref{eq:biharmonicvariatonalform} 的解. 
\end{theorem}
\begin{proof}
首先验证适定性. 双线性形式 $(\cdot, \cdot)$ 在算子 $\operatorname{div}\operatorname{div}: H^{-1}(\operatorname{div}\operatorname{div}, \Omega; \mathbb{S}) \to H^{-1}(\Omega)$ 的核空间上具有强制性：
\[
    (\boldsymbol{\tau}, \boldsymbol{\tau}) = \|\boldsymbol{\tau}\|^2 = \|\boldsymbol{\tau}\|_{H^{-1}(\operatorname{div}\operatorname{div})}^2, \quad \forall~\boldsymbol{\tau} \in H^{-1}(\operatorname{div}\operatorname{div}, \Omega; \mathbb{S}) \cap \ker(\operatorname{div}\operatorname{div}).
\]
结合 inf-sup 条件 \eqref{eq:divdivn1infsup}，由 Brezzi 理论可知，分布混合变分形式 \eqref{eq:hhjmixedformulation} 是适定的. 

接着证明分布混合变分形式 \eqref{eq:hhjmixedformulation} 与原问题 \eqref{eq:biharmonicvariatonalform} 的等价性. 
由于两个问题均存在唯一解，只需证明：若 $u$ 是问题 \eqref{eq:biharmonicvariatonalform} 的解，且令 $\boldsymbol{\sigma} = -\nabla^2 u$，则 $(\boldsymbol{\sigma}, u)$ 满足分布混合变分形式 \eqref{eq:hhjmixedformulation}. 

假设 $u \in H_0^2(\Omega)$ 是问题 \eqref{eq:biharmonicvariatonalform} 的解. 
显然 $\boldsymbol{\sigma} \in L^{2}(\Omega; \mathbb{S})$，且满足：
\[
    (\boldsymbol{\sigma}, \nabla^2 v) = -\langle f, v\rangle, \quad \forall~v \in H_0^2(\Omega).
\]
这意味着在分布意义下 $\operatorname{div}\operatorname{div} \boldsymbol{\sigma} = -f \in H^{-1}(\Omega)$. 
因此，$\boldsymbol{\sigma} \in H^{-1}(\operatorname{div}\operatorname{div}, \Omega; \mathbb{S})$ 且方程 \eqref{eq:hhjmixedformulation2} 成立. 

另一方面，根据 $\operatorname{div}\operatorname{div} \boldsymbol{\tau}$ 的分布定义，有
\[
    \langle \operatorname{div}\operatorname{div} \boldsymbol{\tau}, v\rangle = (\boldsymbol{\tau}, \nabla^2 v), \quad \forall~v \in C_0^{\infty}(\Omega).
\]
由于 $C_0^{\infty}(\Omega)$ 在 $H_0^2(\Omega)$ 中稠密，取 $v = u$ 可得
\[
    \langle \operatorname{div}\operatorname{div} \boldsymbol{\tau}, u\rangle = (\boldsymbol{\tau}, \nabla^2 u) = -(\boldsymbol{\tau}, \boldsymbol{\sigma}),
\]
移项即得 $(\boldsymbol{\sigma}, \boldsymbol{\tau}) + \langle \operatorname{div}\operatorname{div} \boldsymbol{\tau}, u\rangle = 0$，这证明了 \eqref{eq:hhjmixedformulation1} 成立. 
\end{proof}
...
因为 $\div E_{0,k}^{\bot}(\mathbb S)=\div\mathbb B_k(\div, T; \mathbb S)$，所以映射
 ...
 对于 $\boldsymbol v\in E_{0,k}^{\bot}(\mathbb S)$，若对所有 $\boldsymbol q\in \mathbb P_{k-1}(T;\mathbb R^d)$ 都有 $(\boldsymbol v, \defm\boldsymbol q)_T = 0$，则 $\div \boldsymbol v = \boldsymbol 0$，即 $\boldsymbol v\in E_{0,k}(\mathbb S)$. 于是 $\boldsymbol v\in E_{0,k}(\mathbb S)\cap E_{0,k}^{\bot}(\mathbb S) = \{\boldsymbol 0\}$. 因此 $E_{0,k}^{\bot}(\mathbb S)$ 由 $\mathcal N(\defm \mathbb P_{k-1}(T;\mathbb R^d))$ 确定. 由维数匹配可知结论成立. 
\end{proof}
...
基于分布混合变分形式 \eqref{eq:hhjmixedformulation} 的 Hellan-Herrmann-Johnson (HHJ) 混合有限元方法 \cite{Hellan1967,Herrmann1967,Johnson1973} 为： 找 $\boldsymbol{\sigma}_h\in \Sigma_{k-1}^{nn}(\mathcal{T}_h;\mathbb{S})$ 和 $u_h\in \mathring{V}_k^{\grad}(\mathcal{T}_h)$ 满足
...
\begin{equation}\label{eq:HHJinfsup}
    \|v_h\|_{2,h} \lesssim \sup_{\boldsymbol{\tau}_h \in \Sigma_{k-1}^{nn}(\mathcal{T}_h;\mathbb{S})} \frac{((\operatorname{div}\operatorname{div})_w \boldsymbol{\tau}_h, v_h)}{\|\boldsymbol{\tau}_h\|_{0,h}}, \quad \forall~v_h \in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h).
\end{equation}
这表明
\begin{equation}\label{hhjweakdivdivonto}
    (\operatorname{div}\operatorname{div})_w \Sigma_{k-1}^{nn}(\mathcal{T}_h;\mathbb{S}) = \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h).
\end{equation}
\end{lemma}
...
\begin{theorem}
HHJ 混合元方法 \eqref{eq:hhjmfem} 是适定的，且满足
...
\]
结合离散 inf-sup 条件 \eqref{eq:HHJinfsup}，由 Brezzi 理论可得 HHJ 混合元方法 \eqref{eq:hhjmfem} 的适定性.
...
\subsection{二维分布 div\,div 复形与 Hellan-Herrmann-Johnson 复形}

在本节中，我们将给出二维分布 div\,div 复形 \eqref{eq:distribdivdivcomplex2d} 和  Hellan-Herrmann-Johnson (HHJ) 复形 \eqref{eq:hhjcomplex2d}. 


% \subsubsection{符号说明}
对于标量函数~$v$, 定义
$$
\mskw v:=\begin{pmatrix}
0 & -v \\
v & 0
\end{pmatrix},\quad \curl v:=\left(
\frac{\partial v}{\partial x_2},
-\frac{\partial v}{\partial x_1}
\right)^{\intercal} = (\grad v)^{\perp},
$$
其中~$(a,b)^{\perp} := (b, -a)$ 表示顺时针旋转 $90^{\circ}$. 定义 $\hess := \grad\grad$ 以及
$$
\air v:=\curl\curl v=\begin{pmatrix}
\medskip
\frac{\partial^2v}{\partial x_2^2} & -\frac{\partial^2v}{\partial x_1\partial x_2} \\
-\frac{\partial^2v}{\partial x_1\partial x_2} & \frac{\partial^2v}{\partial x_1^2}
\end{pmatrix}.
$$
由此可得恒等式~$\div(\mskw v) = -\curl v$. 

对于向量函数~$\boldsymbol{v}=(v_1, v_2)$, 定义
$$
\rot\boldsymbol{v}:=\frac{\partial v_2}{\partial x_1}-\frac{\partial v_1}{\partial x_2} = \div \bs v^{\perp}.
$$
对于张量函数~$\boldsymbol{\tau}=\begin{pmatrix}
\tau_{11} & \tau_{12} \\
\tau_{21} & \tau_{22}
\end{pmatrix}$，定义
\begin{align*}
\sym\boldsymbol{\tau}:=\frac{1}{2}(\boldsymbol{\tau}+\boldsymbol{\tau}^{\intercal}),\,\;\; \skw\boldsymbol{\tau}:=\frac{1}{2}(\boldsymbol{\tau}-\boldsymbol{\tau}^{\intercal}),\, \;\; \sskw\boldsymbol{\tau}:=\mskw^{-1}\circ\skw\boldsymbol{\tau}=\frac{1}{2}(\tau_{21}-\tau_{12}).
\end{align*}
直接计算可得
\begin{equation}\label{eq:anticommutativeprop1}
\div\boldsymbol{v}=2\,\sskw(\curl \boldsymbol{v}), \quad \rot\boldsymbol{v} = 2\,\sskw(\grad \boldsymbol{v}).
\end{equation}

\begin{lemma}
设 $\Omega \subset \mathbb{R}^2$ 是有界可缩区域. 则成立：
\begin{equation}\label{Hn1Sdivdivker}    
H^{-1}(\div\div, \Omega; \mathbb{S}) \cap \ker(\div\div) = \sym\curl H^1(\Omega; \mathbb{R}^2).
\end{equation}
\end{lemma}
\begin{proof}
首先，容易验证包含关系 $\sym\curl H^1(\Omega; \mathbb{R}^2) \subseteq H^{-1}(\div\div, \Omega; \mathbb{S}) \cap \ker(\div\div)$. 

接下来证明反向包含关系：$H^{-1}(\div\div, \Omega; \mathbb{S}) \cap \ker(\div\div) \subseteq \sym\curl H^1(\Omega; \mathbb{R}^2)$. 

设 $\bs{\tau} \in H^{-1}(\div\div, \Omega; \mathbb{S}) \cap \ker(\div\div)$，则有 $\div\bs{\tau} \in H^{-1}(\Omega; \mathbb{R}^2) \cap \ker(\div)$. 由 de Rham 复形知 $H^{-1}(\Omega; \mathbb{R}^2) \cap \ker(\div) = \curl L^2(\Omega)$，故存在 $w \in L^2(\Omega)$ 使得 $\div\bs{\tau} = \curl w$. 利用恒等式 $\div(\mskw w) = -\curl w$，可得
\[
\div(\bs{\tau} + \mskw w) = 0.
\]
再次利用 $L^{2}(\Omega; \mathbb{R}^2) \cap \ker(\div) = \curl H^1(\Omega)$，可知存在 $\bs{v} \in H^1(\Omega; \mathbb{R}^2)$ 使得 $\bs{\tau} + \mskw w = \curl \bs{v}$. 
注意到 $\bs{\tau}$ 是对称张量（$\bs{\tau} \in \mathbb{S}$），因此
\[
\bs{\tau} = \sym \bs{\tau} = \sym(\bs{\tau} + \mskw w) = \sym\curl \bs{v}.
\]
即 $\bs{\tau} \in \sym\curl H^1(\Omega; \mathbb{R}^2)$. 证毕. 
\end{proof}

\begin{theorem}
设 $\Omega\subset\mathbb R^2$ 是有界可缩区域. 如下二维分布 div\,div 复形
\begin{equation}\label{eq:distribdivdivcomplex2d}
\textrm{RT}\xrightarrow{\hookrightarrow} H^1(\Omega;\mathbb R^2)\xrightarrow{\sym\curl} H^{-1}(\div{\div},\Omega;\mathbb{S}) \xrightarrow{\div\div} H^{-1}(\Omega)\xrightarrow{}0
\end{equation}
是正合的.
\end{theorem}
\begin{proof}
结合 \eqref{divdivHn1Sonto} 的满射性与式 \eqref{Hn1Sdivdivker} 的核空间刻画，要证明复形正合，只需要证明
\begin{equation}\label{eq:symcurlker2d}
H^1(\Omega; \mathbb{R}^2) \cap \ker(\sym\curl) = \textrm{RT}.
\end{equation}
显然 $\textrm{RT} \subseteq H^1(\Omega; \mathbb{R}^2) \cap \ker(\sym\curl)$. 现在我们证明反向包含关系. 

设 $\boldsymbol{v} \in H^1(\Omega; \mathbb{R}^2) \cap \ker(\sym\curl)$，则 $\sym\curl\boldsymbol{v} = 0$. 这意味着 $\curl\boldsymbol{v}$ 是一个反对称矩阵. 
利用关系式 \eqref{eq:anticommutativeprop1}，我们有 $\div\boldsymbol{v} = 2\sskw(\curl \boldsymbol{v})$. 
由于 $\sym\curl\boldsymbol{v} = 0$，这意味着 $\curl\boldsymbol{v}$ 完全由其反对称部分决定，即 $\curl\boldsymbol{v} = \mskw(\frac{1}{2}\div\boldsymbol{v})$. 

注意到 $\div(\curl \boldsymbol{v}) \equiv 0$（按行取散度），故对上式两边取散度可得：
\[
0 = \div\left(\mskw\left(\frac{1}{2}\div\boldsymbol{v}\right)\right) = -\frac{1}{2}\curl(\div\boldsymbol{v}).
\]
这意味着标量函数 $\div\boldsymbol{v}$ 的旋度为零，即 $\div\boldsymbol{v}$ 是常数. 
由此可知，$\curl \boldsymbol{v}$ 是一个常数反对称矩阵. 这意味着 $\grad\boldsymbol{v}$ 是对角矩阵，且对角元是相同的常数. 
因此，$\boldsymbol{v}$ 具有形式 $\boldsymbol{v}(\boldsymbol{x}) = a\boldsymbol{x} + \boldsymbol{b}$，即 $\boldsymbol{v} \in \textrm{RT}$. 证毕. 
\end{proof}

\begin{lemma}\label{lem:divdivwkernel}
设 $\boldsymbol{\tau} \in \Sigma_{k-1}^{nn}(\mathcal{T}_h; \mathbb{S})$ 满足 $(\operatorname{div}\operatorname{div})_w \boldsymbol{\tau} = 0$，则其在分布意义下的双散度 $\operatorname{div}\operatorname{div}\boldsymbol{\tau} = 0$. 
\end{lemma}
\begin{proof}
对任意 $v \in C_0^{\infty}(\Omega)$，利用分布导数的定义以及离散双散度算子的性质 \eqref{eq:bp0}，我们有
\[
\langle \operatorname{div}\operatorname{div}\boldsymbol{\tau}, v \rangle 
= (\boldsymbol{\tau}, \nabla^2 v) = b_h(\boldsymbol{\tau}, v)
= b_h(\boldsymbol{\tau}, I_h^{\operatorname{grad}} v) 
= ((\operatorname{div}\operatorname{div})_w \boldsymbol{\tau}, I_h^{\operatorname{grad}} v) 
= 0.
\]
因此，$\operatorname{div}\operatorname{div}\boldsymbol{\tau} = 0$. 
\end{proof}


\begin{theorem}
设 $\Omega\subset\mathbb R^2$ 是有界可缩区域，且整数 $k\geq1$. 如下 Hellan-Herrmann-Johnson (HHJ) 复形
\begin{equation}\label{eq:hhjcomplex2d}
\textrm{RT}\xrightarrow{\hookrightarrow} V_k^{\grad}(\mathcal{T}_h;\mathbb R^2)\xrightarrow{\sym\curl} \Sigma_{k-1}^{nn}(\mathcal{T}_h;\mathbb{S}) \xrightarrow{(\div\div)_w} \mathring{V}_k^{\grad}(\mathcal{T}_h) \xrightarrow{}0
\end{equation}
是正合的.
\end{theorem}
\begin{proof}
首先考察 $\operatorname{sym}\operatorname{curl}$ 的核空间. 
显然 $\textrm{RT} \subseteq V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)$. 
另一方面，由于 $V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2) \subseteq H^1(\Omega; \mathbb{R}^2)$，故
\[
V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2) \cap \ker(\operatorname{sym}\operatorname{curl}) \subseteq H^1(\Omega; \mathbb{R}^2) \cap \ker(\operatorname{sym}\operatorname{curl}).
\]
利用连续情形下的核空间性质 \eqref{eq:symcurlker2d} 可得 $V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2) \cap \ker(\operatorname{sym}\operatorname{curl}) = \textrm{RT}$. 

接下来结合 \eqref{hhjweakdivdivonto} 的满射性，要证明复形正合，只需证明
\begin{equation}\label{nnSdivdivwker}    
\Sigma_{k-1}^{nn}(\mathcal{T}_h; \mathbb{S}) \cap \ker((\operatorname{div}\operatorname{div})_w) = \operatorname{sym}\operatorname{curl} V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2).
\end{equation}

我们先证明包含关系“$\supseteq$”. 设 $\boldsymbol{\tau} = \operatorname{sym}\operatorname{curl}\boldsymbol{v}$，其中 $\boldsymbol{v} \in V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)$. 
根据弱双散度算子 $(\operatorname{div}\operatorname{div})_w$ 的定义和分部积分公式，对任意 $q \in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h)$ 成立：
\begin{align*}
((\operatorname{div}\operatorname{div})_w \boldsymbol{\tau}, q) 
&= \sum_{T \in \mathcal{T}_h} (\operatorname{curl}\boldsymbol{v}, \nabla^2 q)_T - \sum_{T \in \mathcal{T}_h} (\boldsymbol{n}^{\intercal}(\operatorname{curl}\boldsymbol{v})\boldsymbol{n}, \partial_n q)_{\partial T} \\
&= \sum_{T \in \mathcal{T}_h} (\partial_t(\boldsymbol{t} \cdot \boldsymbol{v}), \partial_t q)_{\partial T} 
= 0.
\end{align*}
最后一个等号成立是因为 $\boldsymbol{v}$ 和 $q$ 的切向导数在单元边界上是连续的（或在边界处为零），边界积分项相互抵消. 
即 $(\operatorname{div}\operatorname{div})_w \boldsymbol{\tau} = 0$. 故 $\operatorname{sym}\operatorname{curl} V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2) \subseteq \Sigma_{k-1}^{nn}(\mathcal{T}_h; \mathbb{S}) \cap \ker((\operatorname{div}\operatorname{div})_w)$. 

现在证明反向包含关系“$\subseteq$”. 
设 $\boldsymbol{\tau} \in \Sigma_{k-1}^{nn}(\mathcal{T}_h; \mathbb{S}) \cap \ker((\operatorname{div}\operatorname{div})_w)$. 由引理~\ref{lem:divdivwkernel} 可知，其分布双散度 $\operatorname{div}\operatorname{div}\boldsymbol{\tau} = 0$. 
根据连续复形的核空间刻画 \eqref{Hn1Sdivdivker}，存在 $\boldsymbol{v} \in H^1(\Omega; \mathbb{R}^2)$ 使得 $\boldsymbol{\tau} = \operatorname{sym}\operatorname{curl}\boldsymbol{v}$. 

在每个单元 $T \in \mathcal{T}_h$ 上，对上式作用散度算子得：
\[
\operatorname{curl}(\operatorname{div}\boldsymbol{v}) = 2\operatorname{div}\boldsymbol{\tau}.
\]
由于 $\boldsymbol{\tau} \in \Sigma_{k-1}^{nn}(\mathcal{T}_h; \mathbb{S})$ 是分片 $k-1$ 次多项式，故 $\operatorname{div}\boldsymbol{\tau} \in \mathbb{P}_{k-2}(T; \mathbb{R}^2)$. 
这表明 $\operatorname{curl}(\operatorname{div}\boldsymbol{v}) \in \mathbb{P}_{k-2}(T; \mathbb{R}^2)$，从而 $\operatorname{div}\boldsymbol{v} \in \mathbb{P}_{k-1}(T)$. 
利用关系式 \eqref{eq:anticommutativeprop1}，我们有 $\operatorname{curl}\boldsymbol{v} = \operatorname{sym}\operatorname{curl}\boldsymbol{v} + \operatorname{skw}\operatorname{curl}\boldsymbol{v} = \boldsymbol{\tau} + \operatorname{mskw}(\frac{1}{2}\operatorname{div}\boldsymbol{v})$. 
由于 $\boldsymbol{\tau} \in \mathbb{P}_{k-1}(T; \mathbb{S})$ 且 $\operatorname{div}\boldsymbol{v} \in \mathbb{P}_{k-1}(T)$，因此 $(\operatorname{curl}\boldsymbol{v})|_T \in \mathbb{P}_{k-1}(T; \mathbb{M})$. 
这意味着 $\nabla \boldsymbol{v}|_T \in \mathbb{P}_{k-1}(T; \mathbb{M})$，即 $\boldsymbol{v}|_T \in \mathbb{P}_{k}(T; \mathbb{R}^2)$. 
综上，由于 $\boldsymbol{v} \in H^1(\Omega)$ 且是分片 $k$ 次多项式，故 $\boldsymbol{v} \in V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)$. 
这也即证明了 $\boldsymbol{\tau} \in \operatorname{sym}\operatorname{curl} V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)$. 
\end{proof}

\subsection{Helmholtz 分解与等价的解偶格式}

\begin{lemma}
设 $\Omega\subset\mathbb R^2$ 是有界可缩区域. 如下 Helmholtz 分解成立：
\begin{equation}\label{eq:distribdivdivHelmdecomp2d}
H^{-1}(\operatorname{div}\operatorname{div},\Omega;\mathbb{S}) = \iota(H_0^1(\Omega)) \oplus \operatorname{sym}\operatorname{curl} H^1(\Omega; \mathbb{R}^2).
\end{equation}
即：对任意 $\boldsymbol{\tau}\in H^{-1}(\operatorname{div}\operatorname{div},\Omega;\mathbb{S})$，存在唯一的 $w\in H_0^1(\Omega)$ 和 $\boldsymbol{v}\in H^1(\Omega; \mathbb{R}^2)/\textrm{RT}$，使得
\begin{equation*}
\boldsymbol{\tau}= \iota(w) + \operatorname{sym}\operatorname{curl} \boldsymbol{v}, \quad \|w\|_1 + \|v\|_{H^1/\textrm{RT}} \lesssim \|\boldsymbol{\tau}\|_{H^{-1}(\operatorname{div}\operatorname{div})}.   
\end{equation*}
\end{lemma}
\begin{proof}
利用直和分解 \eqref{Hn1divdivSdecomp} 以及 $\operatorname{div}\operatorname{div}$ 算子核空间的刻画 \eqref{Hn1Sdivdivker} 即可直接得证.     
\end{proof}

\begin{lemma}[引理 3.1, 文献 \cite{HuangHuangXu2011}]
设 $\Omega\subset\mathbb R^2$ 是有界可缩区域. 如下 Helmholtz 分解成立：
\begin{equation}\label{eq:L2divdivHelmdecomp2d}
L^{2}(\Omega;\mathbb{S}) = \nabla^2 H_0^2(\Omega) \oplus \operatorname{sym}\operatorname{curl} H^1(\Omega; \mathbb{R}^2).
\end{equation}
此分解是 $L^2$ 正交的. 即：对任意 $\boldsymbol{\tau}\in L^{2}(\Omega;\mathbb{S})$，存在 $w\in H_0^2(\Omega)$ 和 $\boldsymbol{v}\in H^1(\Omega; \mathbb{R}^2)$ 使得
\begin{equation*}
\boldsymbol{\tau}= \nabla^2 w + \operatorname{sym}\operatorname{curl} \boldsymbol{v}, \quad \|w\|_2 + \|v\|_1 \lesssim \|\boldsymbol{\tau}\|.   
\end{equation*}
\end{lemma}
\begin{proof}
设 $\boldsymbol{\tau}\in L^{2}(\Omega;\mathbb{S})$. 令 $w\in H_0^2(\Omega)$ 满足
\[
(\nabla^2 w, \nabla^2 v) = (\boldsymbol{\tau}, \nabla^2 v), \quad \forall\, v\in H_0^2(\Omega).
\]
根据 Lax-Milgram 定理，此方程显然是适定的. 令残量 $\boldsymbol{\sigma} = \boldsymbol{\tau} - \nabla^2 w$，则有
\[
(\boldsymbol{\sigma}, \nabla^2 v) = 0, \quad \forall\, v\in H_0^2(\Omega).
\]
这意味着在分布意义下 $\operatorname{div}\operatorname{div} \boldsymbol{\sigma} = 0$，即 $\boldsymbol{\sigma} \in L^{2}(\Omega;\mathbb{S})\cap\ker(\operatorname{div}\operatorname{div})$. 
应用 $\operatorname{div}\operatorname{div}$ 算子核空间的刻画 \eqref{Hn1Sdivdivker}（注意 $L^2$ 包含于 $H^{-1}(\operatorname{div}\operatorname{div})$），可知存在 $\boldsymbol{v}\in H^1(\Omega; \mathbb{R}^2)$ 使得 $\boldsymbol{\sigma} = \operatorname{sym}\operatorname{curl}\boldsymbol{v}$. 证毕. 
\end{proof}

\begin{lemma}
设 $\Omega\subset\mathbb R^2$ 是有界可缩区域. 如下离散 Helmholtz 分解成立：
\begin{equation}\label{eq:hhjHelmdecomp2d}
\Sigma_{k-1}^{nn}(\mathcal{T}_h;\mathbb{S}) = I_h^{nn}\big(\iota(\mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h))\big) \oplus \operatorname{sym}\operatorname{curl} V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2).
\end{equation}
% 即：对任意 $\boldsymbol{\tau}_h \in \Sigma_{k-1}^{nn}(\mathcal{T}_h;\mathbb{S})$，存在 $w_h \in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h)$ 和 $\boldsymbol{v}_h \in V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)$ 使得
% \begin{equation*}
% \boldsymbol{\tau}_h = I_h^{nn}\big(\iota(w_h)\big) + \operatorname{sym}\operatorname{curl} \boldsymbol{v}_h.
% \end{equation*}
\end{lemma}
\begin{proof}
首先证明式 \eqref{eq:hhjHelmdecomp2d} 是直和分解. 
设 $\boldsymbol{\tau} \in I_h^{nn}\big(\iota(\mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h))\big) \cap \operatorname{sym}\operatorname{curl} V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)$. 
即存在 $w \in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h)$ 使得 $\boldsymbol{\tau} = I_h^{nn}(\iota(w))$，且 $\boldsymbol{\tau} \in \ker((\operatorname{div}\operatorname{div})_w)$（由 HHJ 复形 \eqref{eq:hhjcomplex2d} 的正合性可知 $\operatorname{sym}\operatorname{curl}$ 的像在 $(\operatorname{div}\operatorname{div})_w$ 的核中）. 
于是，
\[
(\operatorname{div}\operatorname{div})_w \big( I_h^{nn}(\iota(w)) \big) = 0.
\]
利用交换性性质 \eqref{eq:bpi}，可得 $(\operatorname{div}\operatorname{div})_w (\iota(w)) = 0$. 
根据弱双散度的定义，这意味着：
$$
0=((\div\div)_w(\iota(w)), w)= -|w|_1^2.
$$
故 $w=0$，进而 $\boldsymbol{\tau}=0$. 

接着证明分解的存在性. 设 $\boldsymbol{\tau} \in \Sigma_{k-1}^{nn}(\mathcal{T}_h;\mathbb{S})$. 令 $w \in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h)$ 满足
\[
(\nabla w, \nabla v) = -((\operatorname{div}\operatorname{div})_w \boldsymbol{\tau}, v), \quad \forall\, v \in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h).
\]
显然，此离散 Poisson 问题是适定的，且有 $(\div\div)_w(\iota(w))=(\div\div)_w\boldsymbol{\tau}$. 
利用交换性 \eqref{eq:bpi} 可得 $(\operatorname{div}\operatorname{div})_w \big(I_h^{nn}(\iota(w))\big) = (\operatorname{div}\operatorname{div})_w \boldsymbol{\tau}$. 
从而 $\boldsymbol{\tau} - I_h^{nn}\big(\iota(w)\big) \in \Sigma_{k-1}^{nn}(\mathcal{T}_h; \mathbb{S}) \cap \ker((\operatorname{div}\operatorname{div})_w)$. 
最后，应用 $(\div\div)_w$ 算子核空间的刻画 \eqref{nnSdivdivwker}，可知 $\boldsymbol{\tau} - I_h^{nn}\big(\iota(w)\big) \in\operatorname{sym}\operatorname{curl} V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)$. 证毕. 
\end{proof}


应用 Helmholtz 分解 \eqref{eq:distribdivdivHelmdecomp2d}，我们将分布混合变分形式 \eqref{eq:hhjmixedformulation} 中的张量变量分解为 $\boldsymbol{\sigma}=\iota(w) + \operatorname{sym}\operatorname{curl} \boldsymbol{\phi}$ 和 $\boldsymbol{\tau}=\iota(\chi) + \operatorname{sym}\operatorname{curl} \boldsymbol{\psi}$. 由此，分布混合变分形式 \eqref{eq:hhjmixedformulation} 解耦为如下三个子问题：求 $w\in H_0^1(\Omega)$, $\boldsymbol{\phi}\in H^{1}(\Omega; \mathbb{R}^2)/\textrm{RT}$ 和 $u\in H_0^1(\Omega)$ 使得
\begin{subequations}\label{eq:hhjmixedformdecouple}
\begin{align}
    (\nabla w, \nabla v) &= \langle f, v\rangle, && \forall~v\in H_0^1(\Omega), \label{eq:hhjmixedformdecouple1}\\
    (\operatorname{sym}\operatorname{curl}\boldsymbol{\phi}, \operatorname{sym}\operatorname{curl}\boldsymbol{\psi}) &= - (\iota(w), \operatorname{sym}\operatorname{curl}\boldsymbol{\psi}), && \forall~\boldsymbol{\psi}\in H^{1}(\Omega; \mathbb{R}^2)/\textrm{RT}, \label{eq:hhjmixedformdecouple2}\\
    (\nabla u, \nabla \chi) &= (\boldsymbol{\sigma}, \iota(\chi)), && \forall~\chi\in H_0^1(\Omega). \label{eq:hhjmixedformdecouple3}
\end{align}
\end{subequations}
该系统是解耦的，求解流程如下：首先求解 Poisson 方程 \eqref{eq:hhjmixedformdecouple1} 得到 $w$；接着求解方程 \eqref{eq:hhjmixedformdecouple2} 得到 $\boldsymbol{\phi}$，从而确定 $\boldsymbol{\sigma}$；最后求解 Poisson 方程 \eqref{eq:hhjmixedformdecouple3} 得到 $u$. 
相比于原始变分问题 \eqref{eq:biharmonicvariatonalform}，为解耦后的形式构造协调有限元空间要容易得多. 这是因为系统的阶数降低了，且针对 Poisson 方程的有限元理论与算法已十分成熟. 

此类分解最早由 Krendl 等人 \cite{KrendlRafetsederZulehner2016} 提出，并在 \cite{RafetsederZulehner2018} 中推广至混合边界条件情形. 类似地，混合变分形式 \eqref{eq:hhjmixedformulation} 与解耦形式 \eqref{eq:hhjmixedformdecouple} 之间的等价性亦可推广至三维情形 \cite{PaulyZulehner2020}. 

同样地，应用离散 Helmholtz 分解 \eqref{eq:hhjHelmdecomp2d}，我们将 HHJ 混合元方法 \eqref{eq:hhjmfem} 中的张量变量分解为 $\boldsymbol{\sigma}_h=I_h^{nn}(\iota(w_h)) + \operatorname{sym}\operatorname{curl} \boldsymbol{\phi}_h$ 和 $\boldsymbol{\tau}_h=I_h^{nn}(\iota(\chi_h)) + \operatorname{sym}\operatorname{curl} \boldsymbol{\psi}_h$. 则 HHJ 混合元方法 \eqref{eq:hhjmfem} 解耦为 \cite{KrendlRafetsederZulehner2016,ChenHuHuang2018}：求 $w_h\in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h)$, $\boldsymbol{\phi}_h\in V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)/\textrm{RT}$ 和 $u_h\in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h)$ 使得
\begin{align}
    (\nabla w_h, \nabla v_h) &= \langle f, v_h\rangle, && \forall~v_h\in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h), \label{eq:hhjmixedformdecouple1_h}\\
    (\operatorname{sym}\operatorname{curl}\boldsymbol{\phi}_h, \operatorname{sym}\operatorname{curl}\boldsymbol{\psi}_h) &= - (I_h^{nn}(\iota(w_h)), \operatorname{sym}\operatorname{curl}\boldsymbol{\psi}_h), && \forall~\boldsymbol{\psi}_h\in V_k^{\operatorname{grad}}(\mathcal{T}_h; \mathbb{R}^2)/\textrm{RT}, \label{eq:hhjmixedformdecouple2_h}\\
    (\nabla u_h, \nabla \chi_h) &= (\boldsymbol{\sigma}_h, I_h^{nn}(\iota(\chi_h))), && \forall~\chi_h\in \mathring{V}_k^{\operatorname{grad}}(\mathcal{T}_h). \label{eq:hhjmixedformdecouple3_h}
\end{align}
因此，求解 HHJ 混合元方法 \eqref{eq:hhjmfem} 等价于求解两个 Poisson 方程的 Lagrange 元方法以及一个类线弹性问题的 Lagrange 元方法 \eqref{eq:hhjmixedformdecouple2_h}. 

文献 \cite{ChenHuang2018} 建立了一个通用框架，能够系统地将高阶椭圆方程解耦为 Poisson 型方程与 Stokes 型方程的组合. 其核心思想是在统一的体系下构造涉及相关复形与 Helmholtz 分解的底层交换图表. 对解耦后的变分形式进行离散化，可以自然地导出 Galerkin 投影与解耦近似解之间的超收敛性. 该框架的应用范畴涵盖但不限于重调和方程、四阶旋度方程以及三调和方程的原始变分形式与混合变分形式. 
