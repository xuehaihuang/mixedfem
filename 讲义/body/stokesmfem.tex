...
\end{aligned}
\end{equation}
对所有 $\boldsymbol{v}_h\in \mathring{\mathbb{V}}_{k,\ell}^{\div}$ 成立. 这意味着 $|\cdot|_{1,h}$ 是空间 $\mathring{\mathbb{V}}_{k,\ell}^{\div}$ 上的范数. 
\end{lemma}
\begin{proof}
...
$$
结合 \eqref{eq:weakdevgradnormequiv}，得到
\begin{equation*}
\interleave(\boldsymbol{v}_h,\boldsymbol{\mu}_h)\interleave_{1,h}\lesssim\|(\dev \grad)_w(\boldsymbol{v}_h,\boldsymbol{\mu}_h)\|=\|Q_{k,h}\boldsymbol{\sigma}-\boldsymbol{\sigma}_h\|\leq\|\boldsymbol{\sigma}-I_h^{\rm tn}\boldsymbol{\sigma}\|\lesssim h^{k+1} |\boldsymbol{u}|_{k+2}.
\end{equation*}
